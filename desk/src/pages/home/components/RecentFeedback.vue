    ...
    <div class="flex flex-col rounded-md p-4 min-h-48 grow w-full h-full">
      ...
      >
        <div class="flex flex-col gap-2 max-w-60">
          <div class="text-base font-medium text-ink-gray-7">No feedback</div>
      ...
      <div v-else class="flex flex-col gap-2 h-full w-full mt-4">
        ...
        <div
          ...
          tabindex="0"
          role="region"
          aria-label="Recent feedback carousel"
        >
...
const handleKeydown = (e: KeyboardEvent) => {
  if (e.key === "ArrowLeft") {
    currentIndex.value =
      (currentIndex.value - 1 + feedbacks.value.length) %
...
const getStyle = (index: number) => {
  const len = feedbacks.value.length;
