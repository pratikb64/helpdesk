...
// Ensure at least one time unit is shown by default if no options are provided
const normalizedOptions = computed<Required<Options>>(() => ({
  hours: props.options?.hours ?? true,
  minutes: props.options?.minutes ?? true,
  seconds: props.options?.seconds ?? false, // Default to false for seconds
}));

// If all options are false, default to showing hours and minutes
