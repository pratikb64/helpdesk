<template>
  <SettingsLayoutBase
    ...
    :description="
      __(
        'Manage pre-defined responses that can be used to respond to common customer queries.'
      )
    "
  >
    <template #actions>
          ...
          >
            ...
            >
              <div
                ...
                "
                class="w-full px-2 col-span-5 flex flex-col justify-center h-14"
              >
                <div
              ...
              <div class="flex justify-between items-center w-full pr-2">
                <div>
                  <Switch
                    size="sm"
                    :modelValue="cannedResponse.enabled"
                    @update:modelValue="onToggle(cannedResponse)"
                  />
                </div>
                <div>
                  <Dropdown
...
import { Button, call, createResource, Input, Switch, toast } from "frappe-ui";
...
const onToggle = (cannedResponse) => {
  cannedResponseListData.setValue.submit(
    {
      name: cannedResponse.name,
      enabled: !cannedResponse.enabled,
    },
    {
      onSuccess: () => {
        toast.success(__("Canned response status updated"));
      },
    }
  );
};
