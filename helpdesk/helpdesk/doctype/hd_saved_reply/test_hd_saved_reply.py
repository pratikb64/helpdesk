...
def make_saved_reply(title, message, scope="Global", teams=None, owner=None):
    ...
    if owner:
        ...
        doc.reload()  # Reload to get updated owner
    return doc
...
class TestHDSavedReply(IntegrationTestCase):
    ...
    def setUpClass(cls):
        ...
        # Create test users
        cls.agent1_user = make_user(AGENT1, "Agent One", ["Agent"])
        cls.agent2_user = make_user(AGENT2, "Agent Two", ["Agent"])
        cls.admin_user = make_user(
            ADMIN_AGENT, "Admin Agent", ["Agent", "System Manager"]
        )
        ...
        # Create teams
        cls.team_a = make_team(TEAM_A, [AGENT1])
        cls.team_b = make_team(TEAM_B, [AGENT2])
    ...
    def test_system_manager_has_permission(self):
        """System Manager should have permission to any saved reply."""
        reply = make_saved_reply(
    ...
    def test_agent_manager_has_permission(self):
        """Agent Manager should have permission to any saved reply."""
        # Create a user with Agent Manager role
        agent_manager_email = "saved_reply_agent_manager@test.com"
        make_user(agent_manager_email, "Agent Manager", ["Agent", "Agent Manager"])
        make_agent(agent_manager_email)

        reply = make_saved_reply(
            "Some Reply", "Test message", scope="Personal", owner=AGENT1
        )

        frappe.set_user(agent_manager_email)
        self.assertTrue(frappe.has_permission("HD Saved Reply", doc=reply))
        frappe.set_user("Administrator")
    ...
    def test_agent_can_edit_global_reply_they_own(self):
        """Agent should be able to edit global reply if they are the owner."""
        reply = make_saved_reply(
            "Global Reply", "Original message", scope="Global", owner=AGENT1
        )

        frappe.set_user(AGENT1)
        doc = frappe.get_doc("HD Saved Reply", reply.name)
        doc.message = "Updated by owner"
        doc.save()  # Should not raise
        self.assertEqual(doc.message, "Updated by owner")
        frappe.set_user("Administrator")
